  ...
  methods: {
    ...
    async initData() {
      ...
      await getSkillsList()
        ...
        .catch((err) => {
          console.log("getSkillsList err", err);
          this.tableLoading = false;
        });
    },
    // 重置
    ...
    async onSubmit() {
      ...
      if (this.dialogForm.id != "") {
        // 修改
        await updateSkill(this.dialogForm)
          .then((res) => {
            if (res.status === 200) {
              this.initData(); // 重新獲取List
              this.$q.notify({
        ...
        // 新增
        await addSkill(this.dialogForm)
          .then((res) => {
            if (res.status === 200) {
              this.initData(); // 重新獲取List
              this.$q.notify({
    ...
    async searchKeyword() {
      ...
      //
      this.tableLoading = true;
      await getSkillsKeywordList(this.searchForm.keyword)
        .then((res) => {
          if (res.status === 200) {
            this.tableData = res.data;
            this.page.totalNum = this.tableData.length;
        ...
        .catch((err) => {
          console.log("getSkillsKeywordList err", err);
        });
    },
  },
  created() {
